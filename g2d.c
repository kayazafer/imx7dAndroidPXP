...
//	g2d_fill_param(src_param, src);
	src_param->left      = src->left;
	src_param->top       = src->top;
	src_param->width     = src->stride;
	src_param->height    = src->height;
	src_param->stride    = src->stride * g2d_get_bpp(src->format) >> 3;
	src_param->paddr     = src->planes[0];
	src_param->pixel_fmt = g2d_pxp_fmt_map(src->format);

	out_param = &(pxp_conf.out_param);
//	g2d_fill_param(out_param, dst);
	out_param->left   = dst->left;
	out_param->top    = dst->top;
	out_param->width  = dst->right - dst->left;
	out_param->height = dst->bottom - dst->top;
	out_param->stride = dst->stride * g2d_get_bpp(dst->format) >> 3;
	out_param->paddr  = dst->planes[0];
	out_param->pixel_fmt = g2d_pxp_fmt_map(dst->format);
...
//		g2d_fill_param(third_param, dst);
		third_param->left   = dst->left;
		third_param->top    = dst->top;
		third_param->width  = dst->right - dst->left;
		third_param->height = dst->bottom - dst->top;
		third_param->stride = dst->stride * g2d_get_bpp(dst->format) >> 3;
		third_param->paddr  = dst->planes[0];
		third_param->pixel_fmt = g2d_pxp_fmt_map(dst->format);
